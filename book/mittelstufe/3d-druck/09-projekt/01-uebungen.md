...
:::collapsible{title="Tipps"}
Nutze die Minkowski-Summe aus [Komplexe Transformationen](../03-transformationen/03-komplexe-transformationen): `minkowski()` mit einem `polyhedron` und einer kleinen `sphere`. Den Tetraeder (Dreieckspyramide) kannst du als `polyhedron` mit 4 Punkten und 4 Flächen definieren.
:::
